...
constexpr uint16_t MAX_RPM = 1200;
constexpr uint16_t MIN_RPM = 10;
...
private:
    ...
    void run();
    void stopMotor();
    uint16_t rpmToDelayMicros(uint16_t rpm) const;

    std::thread motor_thread_;
    std::atomic<bool> running_{true};
    std::atomic<int16_t> target_rpm_{0};
...
inline void StepperClass::setSpeed(double target_linear_vel_mps) {
    ...
    target_rpm_ = static_cast<int16_t>(wheel_rps * 60.0);
}

void StepperClass::run() {
    while (running_) {

        int16_t target = target_rpm_;
        ...
        int16_t rpm = std::clamp<int16_t>(std::abs(target), MIN_RPM, MAX_RPM);
        ...
        uint16_t step_delay = rpmToDelayMicros(rpm);
...
inline uint16_t StepperClass::rpmToDelayMicros(uint16_t rpm) const {
    if (rpm == 0) return 0;
    const uint32_t steps_per_second = (static_cast<uint32_t>(rpm) * STEPS_PER_REV * MICROSTEP) / 60U;
